...
const TARGET_CODED_SECONDS = 15 * 3600;  // 15h of wakatime-logged time (gaps capped at 2min)
const DAYS = 14;
const FIRST_DAY = "2026-06-16";
const MAX_BATCH = 100;
...
(async () => {
  ...
  console.log(`generated ${hbs.length} heartbeats, coded=${(coded / 3600).toFixed(2)}h (target 15h)`);
  ...
  const perDay = {};
  hbs.forEach((h) => { const d = new Date(h.time * 1000).toISOString().slice(0, 10); perDay[d] = (perDay[d] || 0) + 1; });
